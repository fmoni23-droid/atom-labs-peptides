...
if (!document.querySelector(".bulk-shipping-callout")) {
  ...
  catalogToolbar.insertAdjacentHTML("afterend", `
    <div class="bulk-shipping-callout">
      ...
      <span>$10 shipping, $5 handling, and $2 processing. Shipping is free on orders $150+.</span>
...
function getCartTotals(entries) {
  ...
  const handlingValue = entries.length > 0 ? HANDLING_FEE : 0;
  const processingValue = entries.length > 0 ? PROCESSING_FEE : 0;
...
function renderCart() {
  ...
  checkoutSummary.innerHTML = entries.length === 0
    ...
    : `
      ...
      <span>${itemCount} item${itemCount === 1 ? "" : "s"} selected. Shipping: ${shippingValue === 0 ? "Free" : formatPrice(shippingValue)}. Handling: ${formatPrice(handlingValue)}. Processing: ${formatPrice(processingValue)}. Total: ${formatPrice(totalValue)}.${savingsValue > 0 ? ` You save ${formatPrice(savingsValue)}.` : ""}</span>
    `;
}
