...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const productLineItems = await Promise.all(checkoutItems.map(async (item) => {
      ...
      const adjustedUnitAmount = Math.max(0, price.unit_amount - PRICE_REDUCTION_CENTS);
      const finalUnitAmount = Math.round(adjustedUnitAmount * (1 - discount));
      ...
      return {
        ...
        subtotal: finalUnitAmount * item.quantity
      };
    }));
    const currency = productLineItems[0]?.price_data.currency || "usd";
    const subtotal = productLineItems.reduce((sum, item) => sum + item.subtotal, 0);
    const lineItems = productLineItems.map(({ subtotal: _subtotal, ...item }) => item);
    ...
    lineItems.push(
      makeFeeLineItem("Handling Fee", HANDLING_FEE_CENTS, currency),
      makeFeeLineItem("Processing Fee", PROCESSING_FEE_CENTS, currency)
    );
