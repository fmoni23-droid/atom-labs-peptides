...
reviewForm?.addEventListener("submit", (event) => {
  ...
  const email = String(data.get("reviewEmail") || "").trim();
  const rating = String(data.get("reviewRating") || "").trim();
  const review = String(data.get("reviewText") || "").trim();
  const subject = encodeURIComponent("AtomLabs customer review");
  const body = encodeURIComponent([
    "AtomLabs customer review",
    "",
    ...
    review,
    "",
    "I confirm this is my honest review and AtomLabs may post it on the website with my display name."
  ].join("\n"));
  reviewStatus.textContent = "Opening email so the review can be sent for approval.";
  window.location.href = `mailto:atomlabsresearch@gmail.com?subject=${subject}&body=${body}`;
});
